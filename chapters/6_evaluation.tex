...
Measurements were collected on the two machines summarised in \fullref{tab:eval-machines}{Table}.
...
\fullref{fig:eval-cdf}{Figure} shows the cumulative distributions of beat and lighting delivery latency. For beat updates the separation is large: the median beat latency is 8\,ms on the loopback Mac configuration but 102\,ms over the LAN, and the tails diverge further still. On the Mac configuration the distribution is right-skewed: the median of 8\,ms sits well below the mean of 23.8\,ms and a 95th percentile of 100\,ms (\fullref{tab:eval-metrics}{Table}), the latter reaching the sub-100\,ms synchronization threshold itself. This is a transport effect, not a compute effect---it tracks the round-trip time, which rises from a median of 3\,ms (loopback) to 42\,ms (LAN), with a 95th percentile of 715\,ms caused by jitter introduced by the TCP/SSH tunnel rather than by the inference device.
...
Lighting delivery latency is less separated than beat latency---medians of 18\,ms and 21\,ms respectively---because lighting frames arrive in bursts produced by a completed inference pass, so their delivery timing is shaped more by the burst structure than by per-message transport. The aggregate figures for all metrics, reporting median, mean, and 95th percentile where available, are collected in \fullref{tab:eval-metrics}{Table}, kept in the two groups motivated above.

\begin{table}[htbp]
\centering
\caption{Aggregated metrics, pooled across three runs per configuration. Server-side metrics isolate the compute device; end-to-end and network metrics additionally reflect the transport path (loopback for Mac, LAN for Windows). Lower is better throughout except for the lighting frame rate. The lighting frame rate is an average; frames are delivered in bursts of roughly twenty, one burst per completed window pass. Medians are omitted (\textemdash) for server-side metrics where they were not aggregated across runs.}
\label{tab:eval-metrics}
\small
\begin{tabular}{@{}lcccccc@{}}
\toprule
 & \multicolumn{3}{c}{\textbf{Mac-local-MPS}} & \multicolumn{3}{c}{\textbf{Windows-CUDA-LAN}} \\
\cmidrule(lr){2-4}\cmidrule(lr){5-7}
\textbf{Metric} & med. & mean & p95 & med. & mean & p95 \\
\midrule
\multicolumn{7}{@{}l}{\emph{Server-side compute (device)}} \\
Skip-BART inference (s)        & \textemdash & 8.10  & 13.04 & \textemdash & 7.43  & 11.64 \\
BeatNet drain (ms)             & \textemdash & 8.67  & 58.74 & \textemdash & 25.99 & 180.84 \\
BeatNet ingest (ms)            & \textemdash & 1.54  & 3.63  & \textemdash & 2.64  & 6.06 \\
Skip enqueue/dequeue (ms)      & \textemdash & 0.01  & 0.01  & \textemdash & 0.01  & 0.02 \\
Server dwell (ms)              & 1.5   & 12.98 & 74.54 & 2.8   & 46.67 & 247.60 \\
\midrule
\multicolumn{7}{@{}l}{\emph{End-to-end / network (device + transport)}} \\
...
Lighting frame rate (Hz, avg)  & \multicolumn{3}{c}{$\approx$2.50} & \multicolumn{3}{c}{$\approx$2.69} \\
\bottomrule
\end{tabular}
\end{table}
...
Three conclusions follow from these measurements. First, the \textbf{beat-synchronization path meets real-time expectations} when the network is local: a median beat-to-light latency of 8\,ms on the Mac configuration (mean 23.8\,ms, 95th percentile 100\,ms; \fullref{tab:eval-metrics}{Table}) is well within the sub-100\,ms window in which audio and visuals are perceived as synchronous for typical beats (\fullref{chap:methodology-requirements-system-design}{Chapter}), though the tail of the distribution reaches the threshold itself. The beat-synchronization path is the part of the system that most needs to be tight, and it is.
