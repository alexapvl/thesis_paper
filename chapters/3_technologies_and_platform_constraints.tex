...
The system depends on two categories of ML models from \fullref{chap:theoretical-background}{Chapter}: a beat tracker (BeatNet+, BEAST, or PLP) for real-time rhythm analysis, and Skip-BART for generative lighting control. Both were developed in Python using PyTorch. Deploying them alongside a web application requires bridging the Python ML ecosystem and the browser runtime.
...
Browser memory limits (1 to 4 GB per tab) were a primary factor in adopting server-side ML inference. Skip-BART's 240 million parameters alongside the Three.js scene, React state, and audio buffers would leave insufficient headroom. WebGL's higher abstraction level compared to native APIs (no compute shaders, less efficient GPU memory management) is unlikely to be a bottleneck for this project's modest stage scene, but constrains future extensions. WebGPU addresses this prospectively (\fullref{sec:ch3-3d-rendering}{Section}).

The Web Audio API's latency floor (10 to 50 ms total from \texttt{baseLatency} plus \texttt{outputLatency}) is additive with WebSocket round-trip time and server inference time. The perceptual thresholds established in \fullref{chap:theoretical-background}{Chapter} indicate that total pipeline latencies below approximately 100 ms are perceived as synchronous, requiring the latency budget to be managed across the entire chain. Cross-browser differences in WebGL extensions, AudioWorklet behaviour, and WebSocket handling led to targeting Chrome/Chromium as the primary runtime.
...
These constraints define the system's design space. On that basis, \fullref{chap:methodology-requirements-system-design}{Chapter} presents the methodological approach, derives the key requirements, and specifies the resulting system design in detail, including module boundaries, runtime data flow, and major design decisions.
