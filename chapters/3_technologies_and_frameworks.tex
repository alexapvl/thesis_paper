...
\label{chap:technologies-frameworks}
...
The visual core of this application is a real-time 3D rendering of a virtual stage with lighting fixtures. WebGL provides the browser's interface to the GPU: a JavaScript API based on OpenGL ES 2.0 that exposes a programmable rendering pipeline without plugins. \footnote{Source: \url{https://threejs.org/docs/}} However, WebGL operates at a low level of abstraction, requiring manual management of vertex buffers, GLSL shader programs, and the rendering state machine, which makes it impractical for application-level development.

Three.js addresses this by providing a high-level, scene-graph-based abstraction over WebGL. As the dominant web 3D library, \footnote{Sources: \url{https://github.com/mrdoob/three.js}; \url{https://www.npmjs.com/package/three}} it provides the building blocks this project requires: scene and camera management, a renderer, and a hierarchy of meshes, materials, and lights. Most relevant to this project is the SpotLight primitive, which exposes parameters for colour, intensity, cone angle, penumbra, distance, decay, and target direction. These map directly to the physical characteristics of stage lighting fixtures (PAR cans, moving heads), making SpotLight a natural abstraction for the virtual stage. \footnote{Source: \url{https://threejs.org/docs/}} Three.js also supports shadow mapping, post-processing pipelines (bloom, colour grading), and fog simulation, providing the visual vocabulary needed for a convincing stage environment.

The principal alternative, Babylon.js, offers a more comprehensive feature set (built-in physics, visual editor, native WebXR) but produces significantly larger bundles, often exceeding 1 MB even with tree-shaking, \footnote{Source: \url{https://forum.babylonjs.com/t/babylon-js-bundle-size/23477}} compared to Three.js's approximately 155 KB gzipped. \footnote{Source: \url{https://bundlephobia.com/result?p=three}} Since this project requires only lighting, camera controls, and scene composition, Three.js's lighter footprint, larger community, and mature React integration (Section 3.5) make it the better fit.

WebGPU, a next-generation API superseding WebGL, has reached production readiness across all major browsers as of late 2025. \footnote{Sources: \url{https://web.dev/blog/webgpu-supported-major-browsers}; \url{https://github.com/gpuweb/gpuweb/wiki/Implementation-Status}} Three.js already supports a WebGPU renderer alongside its WebGL renderer, so choosing Three.js preserves a clear upgrade path without requiring architectural changes. This project targets the WebGL renderer for maximum compatibility.
...
The Web Audio API is a W3C specification that provides a graph-based audio processing architecture in the browser. \footnote{Source: \url{https://www.w3.org/TR/webaudio-1.1/}} Unlike HTML \texttt{} elements, it exposes a modular system of audio nodes connected into a directed processing graph, enabling real-time analysis, transformation, and routing with low latency. \footnote{Source: \url{https://developer.mozilla.org/en-US/docs/Web/API/Web_Audio_API}} An AudioContext manages the graph and processes audio in render quanta of 128 samples (approximately 2.67 ms at 48 kHz). \footnote{Source: \url{https://developer.mozilla.org/en-US/docs/Web/API/AudioContext}} This application supports two audio input modes that produce an identical real-time stream from the perspective of downstream processing. For microphone input, \texttt{getUserMedia()} provides a MediaStream wrapped as a MediaStreamAudioSourceNode. \footnote{Source: \url{https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getUserMedia}} For file-based input, the uploaded audio is decoded into an AudioBuffer and played back through an AudioBufferSourceNode. Crucially, both modes feed the same processing graph frame by frame as audio plays, rather than pre-analysing the entire file. This unified streaming design ensures the system exercises identical real-time constraints regardless of source.

Two processing nodes are central to the pipeline. The AnalyserNode performs a real-time FFT on each audio block, exposing frequency-domain and time-domain data without modifying the audio stream. \footnote{Source: \url{https://developer.mozilla.org/en-US/docs/Web/API/AnalyserNode/fftSize}} It provides spectral data for the frontend visualisation and can supply initial frequency features to complement server-side analysis. The AudioWorklet runs a user-defined processor in a dedicated audio thread, separate from the main JavaScript thread, ensuring audio processing is not interrupted by UI activity. \footnote{Source: \url{https://developer.mozilla.org/en-US/docs/Web/API/Web_Audio_API/Using_AudioWorklet}} In this project, the AudioWorklet extracts raw PCM audio chunks and transmits them to the server via WebSocket for consumption by the beat tracker and Skip-BART model.

The API imposes several design-relevant constraints. Browsers require a user gesture before an AudioContext can be activated (autoplay policy). The total audio output latency, the sum of \texttt{baseLatency} and \texttt{outputLatency}, typically ranges from 10 to 50 milliseconds on standard hardware. \footnote{Source: \url{https://developer.mozilla.org/en-US/docs/Web/API/AudioContext}} Communication between the AudioWorklet thread and the main thread requires message passing via \texttt{port.postMessage()} or SharedArrayBuffers, since the AudioWorklet runs in an isolated scope.
...
FastAPI serves as the server framework, providing native WebSocket support and an asynchronous architecture suitable for real-time streaming. \footnote{Source: \url{https://fastapi.tiangolo.com/}} The server receives raw PCM audio chunks from the browser, feeds them to the beat tracker, passes audio features to Skip-BART, and streams the resulting beat positions and lighting parameters (hue, value) back to the browser.

\subsection{Real-Time Communication}

The hybrid architecture requires a persistent, bidirectional, low-overhead communication channel. The WebSocket protocol satisfies all three requirements: after an initial HTTP handshake, it maintains a full-duplex connection with minimal per-message framing overhead (2 to 14 bytes versus HTTP's 200 to 800 bytes of headers per request). \footnote{Source: \url{https://blog.postman.com/websockets-vs-http-key-differences-explained/}} On localhost, round-trip latency is sub-millisecond, making the network hop negligible relative to model inference time.
...
The frontend must manage three concurrent responsibilities: rendering an interactive 3D stage, presenting UI controls, and processing real-time data from the WebSocket. The application is written in TypeScript for compile-time type safety across its multiple data flows (audio features, WebSocket messages, Three.js scene state). \footnote{Source: \url{https://www.typescriptlang.org/}} React provides the declarative component model for the UI layer (audio source selection, playback controls, fixture configuration). The 3D scene is managed through React Three Fiber (R3F), a React renderer for Three.js that expresses Three.js objects as JSX components without abstracting away or adding overhead to the underlying library. \footnote{Source: \url{https://github.com/pmndrs/react-three-fiber}} Critically, R3F's 3D render loop runs outside React's reconciliation cycle: the scene renders at the GPU's frame rate without being throttled by React's state diffing, while React handles the UI at interaction speed. \footnote{Source: \url{https://www.npmjs.com/package/@react-three/fiber}} R3F's companion library, Drei, \footnote{Source: \url{https://github.com/pmndrs/drei}} provides ready-made components directly applicable to this project: OrbitControls for camera navigation, Environment for ambient lighting, and a SpotLight helper with volumetric cone visualisation.

State management uses Zustand, a subscription-based store from the same Poimandres collective. \footnote{Source: \url{https://github.com/pmndrs/zustand}} Zustand's key property for this architecture is that it can be read and written from outside React components. The WebSocket handler writes incoming lighting parameters directly into the Zustand store; the R3F render loop reads from it every frame via the \texttt{useFrame} hook, updating the Three.js scene without triggering React re-renders. React UI components subscribe only to the state slices they need (BPM display, connection status) and re-render accordingly. This pattern ensures high-frequency data (30 to 50 lighting updates per second) flows from server to scene without passing through React's rendering cycle.
